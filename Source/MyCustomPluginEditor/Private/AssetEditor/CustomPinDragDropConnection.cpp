...
TSharedRef<FCustomPinDragDropConnection> FCustomPinDragDropConnection::New(const TSharedRef<SGraphPanel>& GraphPanel, const FDraggedPinTable& DraggedPins)
{
	TSharedRef<FCustomPinDragDropConnection> Operation = MakeShareable(new FCustomPinDragDropConnection(GraphPanel, DraggedPins));
	Operation->Construct();

	return Operation;
}

void FCustomPinDragDropConnection::OnDrop(bool bDropWasHandled, const FPointerEvent& MouseEvent)
{
...
void FCustomPinDragDropConnection::HoverTargetChanged()
{
	...
	else if (UEdCustomGraphNode* TargetNodeObj = Cast<UEdCustomGraphNode>(GetHoveredNode()))
	{
		TArray<UEdGraphPin*> ValidSourcePins;
		ValidateGraphPinList(/*out*/ ValidSourcePins);
...
FReply FCustomPinDragDropConnection::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
	TArray<UEdGraphPin*> ValidSourcePins;
	ValidateGraphPinList(/*out*/ ValidSourcePins);
...
FReply FCustomPinDragDropConnection::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
void FCustomPinDragDropConnection::OnDragged(const class FDragDropEvent& DragDropEvent)
{
...
void FCustomPinDragDropConnection::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
...
FCustomPinDragDropConnection::FCustomPinDragDropConnection(const TSharedRef<SGraphPanel>& GraphPanelIn, const FDraggedPinTable& DraggedPinsIn)
